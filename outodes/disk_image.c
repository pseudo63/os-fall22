...
void place_file(char *file, int uid, int gid, int nblocks, int inodepos, int block)
{
  ...
  for (char* ipidx = (char *) ip; ipidx < ip + sizeof(struct inode); ipidx++) {
    if (*ipidx != 0) {
...
void recreate_bitmap(int iblocks) {
  ...
  for(int block_index = 0; block_index < iblocks; block_index++) {
    for (struct inode* ip = (struct inode *) (block_index * BLOCK_SZ); 
                    (ip + sizeof(struct inode)) <= (block_index + 1) * BLOCK_SZ; ip++) {
